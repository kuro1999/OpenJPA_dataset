 ...
 * under the License.
 */
package it.uniroma2.isw2.refactored.identifier;
 ...
 *
 * Changes made (without altering external behavior):
 * - Renamed field _conf to conf to follow naming conventions.
 * - Extracted uniqueness logic into a helper to reduce cognitive complexity.
 * - Added a default case to the switch in makeNameValid.
 * - Removed an unused parameter from the private delimit helper and updated callers.
 * - Kept behavior identical to original implementation.
 */
public class DBIdentifierUtilImpl extends IdentifierUtilImpl implements DBIdentifierUtil,
    ...
    public DBIdentifier makeNameValid(String name, NameSet set, int maxLen,
        ...
        switch (nameType) {
            ...
            case DBIdentifierUtil.COLUMN:
                ...
                break;
            default:
                // Keep default behavior explicit: use DEFAULT type
    ...
    public DBIdentifier makeIdentifierValid(DBIdentifier sname, NameSet set, int maxLen,
        ...
        // If reserved word, append a digit (ensuring space for it)
        ...
        // Handle delimited case conversions
    ...
    /**
     * Extracted helper to ensure a name is unique within the provided NameSet.
     * This preserves the original uniqueness algorithm but reduces the complexity
     * of makeIdentifierValid.
     */
    private DBIdentifier ensureUniqueName(DBIdentifier initialName, NameSet set, int maxLen) {
        DBIdentifier validName = initialName;
        int nameLen = validName.getName().length();
        ...
        while (true) {
            ...
            // we need to check known tables/sequences differently.
            if (validName.getType() == DBIdentifierType.TABLE) {
                known = ((SchemaGroup) set).isKnownTable(QualifiedDBIdentifier.getPath(validName));
            } else if (validName.getType() == DBIdentifierType.SEQUENCE) {
                known = ((SchemaGroup) set).isKnownSequence(QualifiedDBIdentifier.getPath(validName));
            } else {
                known = set.isNameTaken(validName);
            }
            ...
            // If we are here, the name is taken; append a version suffix.
            if (version > 1) {
                // truncate previously appended version digits
                validName = DBIdentifier.truncate(validName, chars);
            }

            // Increase chars when version reaches the next power of 10
            ...
            if (!((path.getType() == DBIdentifierType.COLUMN &&
                ...
                for (DBIdentifier dbIdentifier : names) {
                    ...
                    if (!sName.isDelimited()) {
                        dbIdentifier.setName(delimit(sName.getName(), force));
                    }
                }
                return QualifiedDBIdentifier.newPath(names).getName();
            }
        }
        return delimit(name.getName(), force);
    }

    /**
     * Shorten a name using the database dictionary's shortening rules.
     */
    ...
    /**
     * Helper that actually performs delimiting of a simple name string.
     * The original implementation had an unused 'rule' parameter; it has been
     * removed because it was not used and only added noise.
     */
    private String delimit(String name, boolean force) {
        // Delegate to Normalizer to decide how to delimit/quote the name.
        // Keep behavior consistent with previous implementation.
        return Normalizer.delimit(name, force);
    }
}
