...
public class DBIdentifierUtilImpl extends IdentifierUtilImpl implements DBIdentifierUtil,
    ...
    public DBIdentifier getValidForeignKeyIdentifier(DBIdentifier name, Table table, Table toTable, int maxLen) {
        ...
        DBIdentifier sName = makeName(name, table, "F");
    ...
    public DBIdentifier getValidUniqueIdentifier(DBIdentifier name, Table table, int maxLen) {
        ...
        DBIdentifier sName = makeName(name, table, "U");
        return makeIdentifierValid(sName, table.getSchema().getSchemaGroup(),
                maxLen, true);
    }
    ...
    public DBIdentifier getValidIndexIdentifier(DBIdentifier name, Table table, int maxLen) {
        ...
        DBIdentifier sName = makeName(name, table, "I");
        return makeIdentifierValid(sName, table.getSchema().getSchemaGroup(),
                maxLen, true);
    }
    ...
    public DBIdentifier getValidSequenceIdentifier(DBIdentifier name, Schema schema, int maxLen) {
        ...
        DBIdentifier sName = makeName(name, "S");
    ...
    public DBIdentifier getValidTableIdentifier(DBIdentifier name, Schema schema, int maxLen) {
        ...
        DBIdentifier sName = makeName(name, null);
    ...
    public DBIdentifier makeIdentifierValid(DBIdentifier sname, NameSet set, int maxLen,
        ...
        validName = enforceLengthAndReserved(validName, maxLen, rule);
    ...
    /**
     * Enforce max length and reserved word handling.
     */
    private DBIdentifier enforceLengthAndReserved(DBIdentifier name, int maxLen, String rule) {
        DBIdentifier validName = name;
    ...
    /**
     * Ensure the identifier is unique within the provided NameSet.
     */
    ...
    /**
     * Checks whether the given name is taken in the provided NameSet.
     */
    private boolean isNameTaken(NameSet set, DBIdentifier name) {
        if (name.getType() == DBIdentifierType.TABLE) {
            return ((SchemaGroup) set).isKnownTable(QualifiedDBIdentifier.getPath(name));
        } else if (name.getType() == DBIdentifierType.SEQUENCE) {
            return ((SchemaGroup) set).isKnownSequence(QualifiedDBIdentifier.getPath(name));
        } else {
            return set.isNameTaken(name);
        }
    }
    ...
    public String toDBName(DBIdentifier name, boolean delimit) {
        ...
        IdentifierConfiguration idConfig = getIdentifierConfiguration();
        if (idConfig.getSupportsDelimitedIdentifiers() && delimit
                && idConfig.delimitAll() && !name.isDelimited()) {
    ...
    private String toDBName(String rule, String name, boolean delimit) {
        ...
        IdentifierConfiguration idConfig = getIdentifierConfiguration();
        if (idConfig.getSupportsDelimitedIdentifiers() && delimit &&
                idConfig.delimitAll() && !Normalizer.isDelimited(name)) {
    ...
    private DBIdentifier makeName(DBIdentifier name, Table tbl, String prefix) {
        DBIdentifier sName = DBIdentifier.removeLeading(name, IdentifierUtil.UNDERSCORE);
        ...
        String str = combineNames(Normalizer.getNamingConfiguration(), sName.getType().name(),
    ...
    private DBIdentifier makeName(DBIdentifier name, String prefix) {
        DBIdentifier sName = DBIdentifier.removeLeading(name, IdentifierUtil.UNDERSCORE);
