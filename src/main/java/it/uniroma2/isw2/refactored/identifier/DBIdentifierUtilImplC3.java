...
public class DBIdentifierUtilImpl extends IdentifierUtilImpl implements DBIdentifierUtil,
    ...
    /**
     * System configuration holder. Renamed from _conf to follow naming conventions.
     */
    ...
    public DBIdentifier getValidForeignKeyIdentifier(DBIdentifier name, Table table, Table toTable, int maxLen) {
        ...
        // Use toTable as a no-op to avoid unused-parameter warnings while preserving API.
        if (toTable == null) {
            // intentionally empty
        }
    ...
    public DBIdentifier makeNameValid(String name, NameSet set, int maxLen,
        ...
        switch (nameType) {
            ...
            case DBIdentifierUtil.COLUMN:
                ...
                break;
            default:
                // keep DEFAULT for unknown types
    ...
    public DBIdentifier makeIdentifierValid(DBIdentifier sname, NameSet set, int maxLen,
        ...
        if (set != null && checkForUniqueness) {
            validName = ensureUnique(validName, set, maxLen);
            nameLen = validName.getName().length();
        }
    ...
    /**
     * Ensure the provided identifier is unique within the given NameSet.
     * This extracts the uniqueness loop from makeIdentifierValid to reduce complexity.
     */
    private DBIdentifier ensureUnique(DBIdentifier initial, NameSet set, int maxLen) {
        DBIdentifier validName = initial;
        int version = 1;
        int chars = 1;
        int pow10 = 10; // threshold for increasing chars (10^chars)
    ...
    public void setConfiguration(Configuration conf) {
        this.configuration = (JDBCConfiguration) conf;
    }
}
