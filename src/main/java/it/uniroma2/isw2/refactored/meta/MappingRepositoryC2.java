...
public class MappingRepository extends MetaDataRepository {
    ...
    private final Map<Object, QueryResultMapping> results = new HashMap<>();
    ...
    public QueryResultMapping[] getQueryResultMappings() {
        if (_locking) {
            synchronized (this) {
                ...
                return values.toArray(new QueryResultMapping[0]);
            }
        } else {
            Collection<QueryResultMapping> values = results.values();
            return values.toArray(new QueryResultMapping[0]);
    ...
    protected void prepareMapping(ClassMetaData meta) {
        ...
        if (!mapping.getDescribedType().isInterface() && !mapping.isEmbeddedOnly()) {
        ...
        try {
            if (strat == null) {
                strat = JavaTypes.classForName(name, cls,
                        AccessController.doPrivileged(J2DoPrivHelper.getClassLoaderAction(ClassStrategy.class)), false);
            }
    ...
    protected FieldStrategy namedStrategy(FieldMapping field, boolean installHandlers) {
        ...
        try {
            Class<?> c = JavaTypes.classForName(name, field,
                    AccessController.doPrivileged(J2DoPrivHelper.getClassLoaderAction(FieldStrategy.class)), false);
            if (FieldStrategy.class.isAssignableFrom(c)) {
                ...
                Configurations.configureInstance(strat, getConfiguration(), props);
                return strat;
            }
            ...
            if (installHandlers) {
                ...
                Configurations.configureInstance(vh, getConfiguration(), props);
                field.setHandler(vh);
            }
    ...
    protected DiscriminatorStrategy instantiateDiscriminatorStrategy(String name, Discriminator discrim) {
        ...
        try {
            if (strat == null) {
                strat = JavaTypes.classForName(name, discrim.getClassMapping(),
                        AccessController.doPrivileged(J2DoPrivHelper.getClassLoaderAction(DiscriminatorStrategy.class)), false);
            }
    ...
    protected VersionStrategy instantiateVersionStrategy(String name, Version version) {
        ...
        try {
            if (strat == null) {
                strat = JavaTypes.classForName(name, version.getClassMapping(),
                        AccessController.doPrivileged(J2DoPrivHelper.getClassLoaderAction(VersionStrategy.class)), false);
            }
        } catch (Exception e) {
            throw new MetaDataException(_loc.get("bad-version-strategy", version.getClassMapping(), name), e);
        }
