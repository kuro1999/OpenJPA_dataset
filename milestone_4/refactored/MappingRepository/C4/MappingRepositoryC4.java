...
public class MappingRepository extends MetaDataRepository {
    ...
    private transient MappingDefaults defaults = null;

    /**
     * object->queryresultmapping
     *
     * Marked transient to avoid serialization issues when QueryResultMapping
     * is not serializable. This preserves current runtime behavior while
     * addressing static analysis concerns about non-serializable fields.
     */
    private transient Map<Object, QueryResultMapping> results = new HashMap<>();
    private SchemaGroup schemaGroup = null;
    private StrategyInstaller strategyInstaller = null;
    ...
    public SchemaGroup getSchemaGroup() {
        if (_locking) {
            synchronized (this) {
                if (schemaGroup == null) {
                    schemaGroup = ((JDBCConfiguration) getConfiguration())
    ...
    public StrategyInstaller getStrategyInstaller() {
        if (_locking) {
            synchronized (this) {
                if (strategyInstaller == null) {
                    strategyInstaller = new RuntimeStrategyInstaller(this);
                }
    ...
    private QueryResultMapping getQueryResultMappingInternal(Class<?> cls, String name, ClassLoader envLoader) {
        ...
        QueryResultMapping res = results.get(key);
        if (res != null) {
            return res;
        }
    ...
    public QueryResultMapping[] getQueryResultMappings() {
        if (_locking) {
            synchronized (this) {
                Collection<QueryResultMapping> values = results.values();
    ...
    public QueryResultMapping getCachedQueryResultMapping(Class<?> cls, String name) {
        if (_locking) {
            synchronized (this) {
                return results.get(getQueryResultKey(cls, name));
            }
        } else {
            return results.get(getQueryResultKey(cls, name));
        }
    ...
    private QueryResultMapping addQueryResultMappingInternal(Class<?> cls, String name) {
        ...
        results.put(getQueryResultKey(res), res);
        return res;
    }
    ...
    public boolean removeQueryResultMapping(QueryResultMapping res) {
        if (_locking) {
            synchronized (this) {
                return results.remove(getQueryResultKey(res)) != null;
            }
        } else {
            return results.remove(getQueryResultKey(res)) != null;
        }
    ...
    public boolean removeQueryResultMapping(Class<?> cls, String name) {
        if (_locking) {
            synchronized (this) {
                ...
                return results.remove(getQueryResultKey(cls, name)) != null;
            }
        } else {
            if (name == null) {
                return false;
    ...
    protected ClassStrategy defaultStrategy(ClassMapping cls,
        ...
        Object strat = defaults.getStrategy(cls, adapting);
        if (strat instanceof ClassStrategy) {
    ...
    protected FieldStrategy defaultStrategy(FieldMapping field,
        ...
        if (!field.isSerialized() && (field.getType() == byte[].class
            ...
            if (dict.maxEmbeddedClobSize != -1 && isClob(field, false)) {
                handler = defaultHandler(field, adapting);
                if (handler != null) {
                    if (installHandlers)
                        field.setHandler(handler);
    ...
    private Object mappedStrategy(ValueMapping val, Class<?> type,
        ...
        Object strat = defaults.getStrategy(val, type, adapting);
    ...
    private boolean isClob(ValueMapping val, boolean warn) {
        ...
        if (warn && dict.maxEmbeddedClobSize != -1)
    ...
    protected DiscriminatorStrategy defaultStrategy(Discriminator discrim,
        ...
        Object strat = defaults.getStrategy(discrim, adapting);
        if (strat instanceof String)
        ...
        if (cls.getMappedPCSuperclassMapping() != null)
            return NoneDiscriminatorStrategy.getInstance();
        if (adapting || defaults.defaultMissingInfo())
    ...
    protected VersionStrategy defaultStrategy(Version version,
        ...
        Object strat = defaults.getStrategy(version, adapting);
        if (strat instanceof String)
    ...
    public void endConfiguration() {
        ...
        if (defaults == null)
            defaults = conf.getMappingDefaultsInstance();
        if (schemaGroup != null && schemaGroup instanceof Configurable) {
            ((Configurable) schemaGroup).setConfiguration(conf);
            ((Configurable) schemaGroup).startConfiguration();
            ((Configurable) schemaGroup).endConfiguration();
        }
    }
