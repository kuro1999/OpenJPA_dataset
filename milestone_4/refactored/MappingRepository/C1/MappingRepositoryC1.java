...
public class MappingRepository extends MetaDataRepository {
    ...
    private transient Map<Object, QueryResultMapping> _results = new HashMap<>();
    ...
    protected FieldStrategy defaultStrategy(FieldMapping field,
        ...
        if (field.isSerialized()) {
            if (_dict.maxEmbeddedBlobSize != -1) {
                ...
                applyHandlerIfRequested(field, handler, installHandlers);
    ...
    /**
     * Small helper to apply a handler to a field only if requested.
     */
    private void applyHandlerIfRequested(FieldMapping field, ValueHandler handler, boolean installHandlers) {
        if (handler != null && installHandlers) {
            field.setHandler(handler);
        }
    }
    ...
    protected FieldStrategy defaultTypeStrategy(FieldMapping field,
        ...
        switch (field.getTypeCode()) {
            ...
            default:
                // explicit default to make intent clear and satisfy static analysis
                break;
        }
        return null;
    }
    ...
    public boolean hasJoinColumn(FieldMapping field) {
        return field.getValueInfo().getColumns().size() > 0;
    }

    public boolean hasJoinTable(FieldMapping field) {
        return !DBIdentifier.isNull(field.getMappingInfo().getTableIdentifier());
    }
    ...
    protected ValueHandler defaultHandler(ValueMapping val, boolean adapting) {
        ...
        switch (val.getTypeCode()) {
            ...
            default:
                // explicit default to satisfy static analysis and make intent clear
                break;
        }
        if (!getConfiguration().getCompatibilityInstance()
                .getStoreMapCollectionInEntityAsBlob()
